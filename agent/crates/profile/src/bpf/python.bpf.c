...
int BPF_KPROBE(cuda_malloc_hook, void **address, size_t size)
{
    ...
    bpf_printk("pid %d tgid %d", id & 0xFFFFFFFF, id >> 32);
    bpf_printk("allocate %lu bytes to %lx@%lx", size, allocated, address);
...
int BPF_KRETPROBE(cuda_malloc_ret_hook, long ret)
{
    ...
    bpf_printk("pid %d tgid %d", id & 0xFFFFFFFF, id >> 32);
    bpf_printk("allocated %lu to %lx@%lx", info.size, info.address, data->address);
