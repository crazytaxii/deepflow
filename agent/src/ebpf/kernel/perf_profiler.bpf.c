...
MAP_HASH(dwarf_shard_table, __u32, ShardInfoList, 128)
MAP_HASH(dwarf_unwind_table, __u32, UnwindEntryShard, 40)
...
static inline __attribute__((always_inline)) __u32 find_shard_id(ShardInfoList *shard_info, __u64 pc) {
#pragma unroll
	for (int i = 0; i < 32; i++) {
		ShardInfo *si = shard_info->info + i;
...
static inline __attribute__((always_inline)) __u32 search_uentry(UnwindEntryShard *shard, __u64 pc) {
...
PROGPE(dwarf_unwind)(struct bpf_perf_event_data *ctx) {
	...
	ShardInfoList *shard_info = dwarf_shard_table__lookup(&state->key.tgid);
	if (shard_info == NULL) {
		return 0;
	}
...
#pragma unroll
	for (int i = 0; i < STACK_FRAMES_PER_RUN; i++) {
		...
		UnwindEntryShard *shard = dwarf_unwind_table__lookup(&shard_id);
		if (shard == NULL) {
			return 0;
		}
		...
		__u64 cfa = 0;
		UnwindEntry *entry = &shard->entries[index];
