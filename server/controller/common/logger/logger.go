...
// It adds support for logging with flexible prefixes. Prefixes should implement the LoggerPrefix interface.
...
// Example:
//
//	// logger is a *Logger, NewORGPrefix is a function that returns a LoggerPrefix
//	logger.Info("message", NewORGPrefix(1), NewORGPrefix(12))
//
//	// [ORGID=1] is returned by LoggerPrefix's Prefix() method
//	will log: "[ORGID=1] [ORGID=12] message"
type Logger struct {
	*logging.Logger
}
...
func (l *Logger) splitArgs(args ...interface{}) (trueArgs []interface{}, prefixes string) {
	for _, arg := range args {
		if prefix, ok := arg.(LoggerPrefix); ok {
			prefixes += argsJoiner + prefix.Prefix()
		} else {
...
func (l *Logger) formatArgs(args ...interface{}) []interface{} {
	...
	if prefixes != "" {
		args = append([]interface{}{prefixes}, args...)
	}
	return args
}
