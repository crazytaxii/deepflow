...
import (
	...
	"github.com/deepflowio/deepflow/server/controller/common/logger"
...
func NewRecorder(ctx context.Context, cfg config.RecorderConfig, eventQueue *queue.OverwriteQueue, orgID int, domainLcuuid string) *Recorder {
	...
	if err := md.DB.Where("lcuuid = ?", domainLcuuid).First(&domain).Error; err != nil {
		log.Errorf("domain lcuuid: %s, failed to get domain from db: %s", domainLcuuid, err.Error(), logger.NewORGPrefix(orgID))
		return nil
	}
	md.SetDomain(domain)
